...
export default function MobileTable({ 
  ...
  const getHighPriorityColumns = () => columns.filter(col => !col.mobilePriority || col.mobilePriority === 'high');
  const getRemainingColumns = () => columns.filter(col => col.mobilePriority && col.mobilePriority !== 'high');
                ...
                <div className="grid grid-cols-1 gap-3">
                  {getRemainingColumns().map((column, colIndex) => (
                    <div key={colIndex} className="flex justify-between items-center">
                      <span className="text-xs text-gray-500 mb-1">{column.label}</span>
